...
namespace _1brc
{
    ...
    public static unsafe class DictionaryHelpers
    {
        public static Stats* GetOrCreate(this IStationDictionary d, ReadOnlySpan<byte> name)
        {
            if (name.Length < 32)
            {
                return d.GetOrCreate(SpanToVector(name), name.Length);
            }
            else
            {
                Debug.Assert(name.Length <= 100);
                return d.GetOrCreate(SpanToVector(name),
